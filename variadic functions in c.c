...
int  sum (int count,...) {
    int sum=0,i;
    va_list sumpointer;
    va_start(sumpointer,count);
    for(i=0;i<count;i++)
    {
        sum=sum+va_arg(sumpointer,int);
    }
    va_end(sumpointer);
    return sum;
}
...
int min(int count,...) {
    int min=MAX_ELEMENT,i,result;
    va_list minpointer;
    va_start(minpointer,count);
    for(i=0;i<count;i++)
    {
        result=va_arg(minpointer,int);
        if(min>result)
        {
            min=result;
        }
...
int max(int count,...) {
    int max=MIN_ELEMENT,i,result;
    va_list maxpointer;
    va_start(maxpointer,count);
    for(i=0;i<count;i++)
    {
        result=va_arg(maxpointer,int);
        if(max<result)
        {
            max=result;
        }
